...
module.exports = async function(deployer, network, accounts) {
    if(network != "test") {
        ...
        //add accounts[1] to while list
        txReceipt = await iHEODao.proposeVote(1, 0, KEY_FUNDRAISER_WHITE_LIST, [accounts[1]], [1], 259201, 51,
            {from: accounts[0]});
        console.log("Proposed vote to add accounts[1] to WL");
        events = txReceipt.logs;
        if(events && events.length > 0) {
            console.log("got events");
            console.log(events[0]);
        ...
        //add accounts[2] to while list
        txReceipt = await iHEODao.proposeVote(1, 0, KEY_FUNDRAISER_WHITE_LIST, [accounts[2]], [1], 259201, 51,
            {from: accounts[0]});
        console.log("Proposed vote to add accounts[2] to WL");
        events = txReceipt.logs;
        if(events && events.length > 0) {
            console.log("got events");
            console.log(events[0]);
